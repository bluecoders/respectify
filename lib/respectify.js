...
function isInvalid(obj, prop, spec) {
  ...
  debug('[isInvalid] prop=`%s` val=`%s` allowed=`%s` type=`%s`', prop, val, dt, type)
  ...
  if (~dt.indexOf('string') && type === 'string') {
    if (val) ok = true
  }
  ...
  debug('[isInvalid] invalid prop=`%s` val=`%s` allowed=`%s` recieved=`%s`', prop, val, dt, received)
...
Respect.prototype.normalizeParamFilter = function(filter) {
  if (!filter) {
    return false
  }
  // Single input target
...
Respect.prototype.middleware = function(options) {
  ...
  return function(req, res, next) {
    ...
    spec.parameters.forEach(function(param) {
      var pName = param.name.toLowerCase()
        , err
      ...
      // No param value supplied
      if (!req[use].hasOwnProperty(pName) || typeof req[use][pName] === 'undefined') {

        // Missing required param, check this before `default` as
        // the two are conflicting concepts
        if (param.required) {
          err = new restify.MissingParameterError('Param `' + prop + '` required')
        ...
        // otherwise, consider valid since its optional and remove from obj
          ...
          req[use][pName] = typeof param.default === 'function'
            ? param.default(req, param)
            : param.default
        } else {
          delete req[use][pName]
        }
      }

      // Check for param input errors, add to list if found
      err = err || isInvalid(req[use], pName, param)

      // Check for custom validation function
      if (!err && param.validate) {
        err = param.validate(req[use][pName], req, param)
      }

      if (err) {
        invalid.push(err)
      } else {
        // Check for post-validation transformation function
        if (param.transform) {
          req[use][pName] = param.transform(req[use][pName], req, param)
        }

        // Check for restify-like parameter mapping
        if (options.mapParams) {
          // Map querystring and post data to the `params` object
          if (use !== 'params' && req[use].hasOwnProperty(pName)) {
...
Respect.prototype.getSpecByRoute = function(route) {
  ...
  if (this.globalParams) {
    for (var prop in this.globalParams) {
      optional[prop] = this.globalParams[prop]
    }
  }
...
Respect.prototype.getRouteParams = function(path, version) {
  var params = {}
...
Respect.prototype.getMergedParams = function(path, version, params) {
