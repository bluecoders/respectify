...
Respectify.isInvalid = function(obj, prop, spec) {
  var val = obj[prop]
  ...
  if (~dt.indexOf('array') && val) {
    ...
    if (ok && dt.length > 1 && Array.isArray(val)) {
      ...
      for (var i = 0; i < val.length; i++) {
        var err = Respectify.isInvalid(val, i, {
...
Respectify.prototype.middleware = function(options) {
  ...
  return function(req, res, next) {
    ...
    spec.parameters.forEach(function(param) {
      ...
      // No param value supplied
      if (!req[use].hasOwnProperty(pName)) {
        ...
        // Param not sent, check for required param, 
        } else if (param.required) {
          err = new restify.MissingParameterError('Param `' + pName + '` required')
        }
      }

      // Short out if not sent
      if (!req[use].hasOwnProperty(pName)) return

      // Check for param input errors, add to list if found
      err = err || Respectify.isInvalid(req[use], pName, param)

      // Check for custom validation function
      if (param.validate) err = err || param.validate(req[use][pName], req, param)

      // Stop if any errors
      if (err) return invalid.push(err)
      ...
      if (param.transform) req[use][pName] = param.transform(req[use][pName], req, param)
