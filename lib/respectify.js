...
Respectify.factory = (server, options) => new Respectify(server, options);

Respectify.isInvalid = (obj, prop, spec, req, prefix) => {
	let msg;
	let err;
	let val = obj[prop];
	...
	if (!_.has(obj, prop) && spec.required) {
		const err = new restify.MissingParameterError(`Param \`${label}\` required`);
		...
		if (type === 'array') {
			...
			try {
				...
				if (dt.indexOf('string') < 0) {
					...
					err = new restify.InvalidArgumentError('%s', msg);
					return errorWrap(err, spec, val);
				}
			}
	...
	if (dt.indexOf('number') >= 0 && !ok) {
		...
		if ((type === 'string' || type === 'number') && !isNaN(Number(val))) {
			...
			if ((min && Number(val) < spec.min) || (max && Number(val) > spec.max)) {
				...
				const err = new restify.InvalidArgumentError('%s', msg);
				return errorWrap(err, spec, val);
			}
			obj[prop] = Number(val);
			ok = true;
...
Respectify.prototype.validateTarget = target => {
...
Respectify.prototype.normalizeParamFilter = filter => {
	if (!filter) {
		return false;
	}
	// Single input target
...
Respectify.prototype.middleware = (options = {}) => {
	const self = this;
...
Respectify.prototype.getSpecByRoute = route => {
...
Respectify.prototype.getDefaults = (path, version) => {
	const defaults = {};
...
Respectify.prototype.loadSpecs = version => {
	const self = this;
	let resp = [];
...
Respectify.prototype.findRoutes = (path, version) => {
	const matched = [];
...
Respectify.prototype.findSpecs = (path, version) => {
	let self = this;
...
Respectify.prototype.find = (path, version) => {
	let self = this;
	const routes = this.findRoutes(path, version);
...
Respectify.prototype.getVersions = () => {
...
Respectify.prototype.getRouteParams = (path, version) => {
	const params = {};
...
Respectify.prototype.getMergedParams = (path, version) => {
