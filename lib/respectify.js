...
function isInvalid(obj, prop, spec) {
  ...
  if (dv && dv.length) {
    ...
    // Check for an array that contains the valid values
    if (~dt.indexOf('array') && type === 'string' && ~val.indexOf(',')) {
      var tmp = val.split(',')
        , ok = true
  ...
  return function(req, res, next) {
    ...
    spec.parameters.forEach(function(param) {
      ...
      if (!err && param.validate) {
        err = param.validate(req[use][param.name], req)
      }
      ...
      if (err) {
        ...
        if (param.transform) {
          var transform = param.transform(req[use][param.name])
          if (typeof transform !== 'undefined') {
            req[use][param.name] = transform
          }
        }
...
Respect.prototype.getMergedParams = function(path, version, params) {
  ...
  return _.extend(baseParams || {}, params)
}
