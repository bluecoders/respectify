...
function Respect(server, options) {
  ...
  opt.paramTarget || (opt.paramTarget = 'params')
  ...
  this.validateTarget(opt.paramTarget)
...
Respect.prototype.middleware = function(req, res, next) {
  var invalid = []
    ...
    , target = this.options.paramTarget
    , reqParams = {}
  ...
  spec.parameters.forEach(function(param) {
    var err = isInvalid(req[target], param.name, param)
    if (err) {
      invalid.push(err)
    }
  })
  ...
  for (var prop in req[target]) {
    if (!~validProps.indexOf(prop)) {
      delete req[target][prop]
    }
  }
