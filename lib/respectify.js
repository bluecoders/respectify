...
Respectify.isInvalid = function(obj, prop, spec, req, prefix) {
  ...
  if (!_.has(obj, prop) && spec.required) {
    var err = new restify.MissingParameterError('Param `' + label + '` required')
  ...
  if (~dt.indexOf('array') && val) {
    ...
    // Assume serialized array, attempt to JSON decode
      ...
      try {
        ...
        if (!~dt.indexOf('string')) {
          ...
          var err = new restify.InvalidArgumentError('%s', msg)
          return errorWrap(err, spec, val)
        }
      }
    ...
    if (ok && dt.length > 1 && Array.isArray(val)) {
      var subSpec = _.extend({}, _.omit(spec, [
        'dataTypes'
      ]), {
  ...
  if (~dt.indexOf('object') && type !== 'array') {
    if (type === 'object') {
      obj[prop] = val
      ok = true
    ...
    if (ok && spec.params) {
      var errs = _.compact(spec.params.map(function(param) {
        return Respectify.isInvalid(
          obj[prop]
        , param.name
        , param
...
Respectify.prototype.middleware = function(options) {
  ...
  return function(req, res, next) {
    var invalid = []
      ...
      , reqParams = {}
      , hasParams = _.has(spec, 'parameters')
