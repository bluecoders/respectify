...
var semver = require('semver')
  ...
  , utils = require('./utils')
  , tmpl = require('./tmpl')
  , specTemplate = fs.readFileSync(__dirname + '/templates/spec.md', 'utf-8')
  , restify
...
Respect.prototype.__getRouteSpec = function(route) {
  ...
  for (var name in optional) {
    ...
    // Add the parameter description if one was provided
    if (data.description || data.desc) {
      param.description = data.description || data.desc
    }

    // Add the parameter notes if provided
    if (data.notes) {
      param.notes = Array.isArray(data.notes) ? data.notes : [data.notes]
    }
  ...
  // Add the route description if one was provided
  if (spec.description || spec.desc) {
    def.description = spec.description || spec.desc
  }
 ...
 *
 * @param {String} route version (optional)
 * @param {String} underscore-style template (optional)
 * @return {String} generated markdown
 */

Respect.prototype.generateDocs = function(version, template) {
  return tmpl(template || specTemplate, {
    specs: this.loadSpecs(version)
  })
}
